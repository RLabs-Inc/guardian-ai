...
import {DEFAULT_EXCLUSIONS} from '../../utils/defaultExclusions.js';
...
export class EmergentIndexingService implements IEmergentIndexingService {
	...
	constructor(
		...
		this.dataFlowAnalyzer =
			dataFlowAnalyzer || new DataFlowAnalyzer(this.patternDiscovery);
		// For backwards compatibility we allow a null dependency analyzer, but it's not recommended
	...
	async updateUnderstanding(
		...
		const understanding: CodebaseUnderstanding = {
			...
			dataFlow: existingUnderstanding.dataFlow || {
				nodes: new Map(),
				flows: [],
				paths: [],
			},
		...
		// Build the file system tree
		await this.buildFileSystemTree(
			rootPath,
			newFileSystem,
			mergedOptions as EmergentIndexingOptions,
		);
		...
		console.log(
			`Changes detected: ${comparison.added.length} added, ${comparison.modified.length} modified, ${comparison.deleted.length} deleted`,
		);
		...
		if (
			...
			return {
				...
				stats: {
					...
					dataFlowsDiscovered:
						existingUnderstanding.dataFlow?.flows.length || 0,
					dataFlowPathsIdentified:
						existingUnderstanding.dataFlow?.paths.length || 0,
				},
			};
		}
		...
		// 4. Update patterns
		if (understanding.fileSystem.fileCount > 0) {
			understanding.patterns = await this.patternDiscovery.discoverPatterns(
				understanding,
			);
		}
		...
		understanding.relationships =
			await this.relationshipDetector.detectRelationships(understanding);

		// 6. Update semantic information if enabled
		if (mergedOptions.semanticAnalysis) {
			const semanticResults = await this.semanticAnalyzer.analyzeSemantics(
				understanding,
			);
	...
	private async updateLanguageInfo(
		...
		understanding.languages = await this.languageDetector.detectLanguages(
	...
	private async buildFileSystemTree(
		...
		const fsObject =
			'languages' in fileSystem ? fileSystem.fileSystem : fileSystem;
		...
		console.log(
			`File system tree built: ${fsObject.fileCount} files, ${fsObject.directoryCount} directories`,
		);
	}
	...
	private async detectLanguages(
		...
		console.log(
			`Languages detected: ${understanding.languages.languages.size}`,
		);
	}
	 ...
	 * @private
	 */
	private async extractCodeStructure(
		understanding: CodebaseUnderstanding,
	): Promise<void> {
		...
		console.log(
			`Code structure extracted: ${understanding.codeNodes.size} nodes`,
		);
	}
	...
	private async detectRelationships(
		...
		console.log(
			`Relationships detected: ${understanding.relationships.length}`,
		);
	}
	...
	private async analyzeSemantics(
		...
		console.log(
			`Semantic analysis complete: ${understanding.concepts.length} concepts, ${understanding.semanticUnits.length} semantic units`,
		);
	}
	...
	private async processDataFlows(
		...
		understanding.dataFlow = await this.dataFlowAnalyzer.analyzeDataFlows(
			understanding,
			{
		...
		console.log(
			`Data flow analysis complete: ${understanding.dataFlow.flows.length} flows, ${understanding.dataFlow.paths.length} paths`,
		);
	}
	...
	async clusterCodeElements(
		...
		const clusters = await this.codeElementClustering.clusterCodeElements(
			understanding,
			options,
		);
	...
	async analyzeDataFlows(
		...
		// Use the data flow analyzer to discover data flows
		const dataFlowGraph = await this.dataFlowAnalyzer.analyzeDataFlows(
			understanding,
			options,
		);
		...
		console.log(
			`Discovered ${dataFlowGraph.flows.length} data flows and ${dataFlowGraph.paths.length} data flow paths`,
		);
	...
	async analyzeDependencies(
		...
		console.log('Analyzing dependencies explicitly...');

		// If no dependency analyzer is available, return an empty dependency graph
		if (!this.dependencyAnalyzer) {
			console.warn('No dependency analyzer available, returning empty dependency graph');
			return {
		...
		// Use the dependency analyzer to discover dependencies
		const dependencyGraph = await this.dependencyAnalyzer.analyzeDependencies(understanding);

		// Update the understanding with the discovered dependencies
		understanding.dependencies = dependencyGraph;
		understanding.updatedAt = new Date();

		console.log(
			`Discovered ${dependencyGraph.dependencies.size} dependencies, ${dependencyGraph.imports.length} imports, and ${dependencyGraph.exports.length} exports`
		);
