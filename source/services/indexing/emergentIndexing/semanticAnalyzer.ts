...
export class SemanticAnalyzer implements ISemanticAnalyzer {
  ...
  async analyzeSemantics(understanding: CodebaseUnderstanding): Promise<SemanticResults> {
    ...
    // Calculate sensitivity multipliers based on codebase size
    // adaptiveThresholds can be used in the future to dynamically adjust thresholds
    // based on codebase size
    // const adaptiveThresholds = this.calculateAdaptiveThresholds(understanding);
  ...
  private async extractFileModuleConcepts(understanding: CodebaseUnderstanding): Promise<Concept[]> {
    ...
    // Lower threshold to capture more concepts from smaller codebases
    const fileSignificanceThreshold = Math.max(2, Math.ceil(totalNodes * 0.015)); 
    
    for (const [word, {count, nodes, importance}] of Object.entries(wordCounts)) {
      // Lower importance threshold to capture more concepts
  ...
  private splitIdentifier(identifier: string): string[] {
    ...
    if (identifier.length <= 3 && this.isTechnicalAbbreviation(lowerIdentifier)) {
    ...
    if (abbreviationMatches) {
      for (const abbr of abbreviationMatches) {
        ...
        if (!isAtStart && !isAtEnd) {
          ...
          currentIdentifier = currentIdentifier.replace(abbr, `_${abbr.toLowerCase()}_`);
        }
      }
    }
    ...
    if (/^[a-zA-Z][a-zA-Z0-9]*$/.test(currentIdentifier) && /[A-Z]/.test(currentIdentifier)) {
    ...
    // Process each word to check for known abbreviations
    for (const word of words) {
  ...
  private isCommonWord(word: string): boolean {
    ...
    // Skip checking technical abbreviations if the word is a recognized abbreviation
  ...
  private isTechnicalAbbreviation(word: string): boolean {
    ...
    const technicalAbbreviations = [
      ...
      'ai', 'ml', 'nlp', 'api', 'gui', 'ui', 'ux', 'sdk', 'cli', 'css', 'dom',
      'ssl', 'tls', 'jwt', 'io', 'db', 'orm', 'sql', 'fk', 'pk', 'cdn', 'dns',
      'ip', 'tcp', 'udp', 'http', 'url', 'uri', 'rest', 'spa', 'pwa', 'ssr',
      'csr', 'ci', 'cd', 'vm', 'os', 'fs', 'cpu', 'ram', 'ssd', 'hdd', 'api',
      
      // Development abbreviations
      'dev', 'prod', 'qa', 'uat', 'src', 'lib', 'doc', 'docs', 'env', 'var',
      
      // Design pattern abbreviations
      'mvc', 'mvp', 'mvvm', 'di', 'ioc', 'dao', 'dto', 'vo', 'ddd', 'tdd', 'bdd',
      
      // AI/ML abbreviations
      'nn', 'cnn', 'rnn', 'lstm', 'gru', 'gan', 'nlp', 'ocr', 'tts', 'stt', 'lm',
      
      // Project-specific abbreviations
      'llm', 'rag', 'gu', 'ug', 'uf', 'au', 'ur'
    ];
  ...
  private extractKeyTermsFromComment(comment: string): string[] {
    ...
    const capitalizedPhraseMatches = comment.match(/[A-Z][a-z]+(?:\s+[A-Z][a-z]+)+/g);
    if (capitalizedPhraseMatches) {
      terms.push(...capitalizedPhraseMatches.map(phrase => phrase.toLowerCase()));
    }
  ...
  // Adaptive threshold calculation is implemented but commented out for future use
  // This will be integrated in a future version to make the analyzer automatically
  // adjust sensitivity based on codebase size
  /*
  private calculateAdaptiveThresholds(understanding: CodebaseUnderstanding): Record<string, number> {
    // Get metrics that indicate codebase size
    const fileCount = understanding.fileSystem.fileCount;
    const nodeCount = understanding.codeNodes.size;
    
    // Base sensitivity on file count (inverse relationship - smaller = more sensitive)
    const fileSensitivity = this.calculateSizeSensitivity(fileCount);
    
    // Also consider node counts
    const nodeDensity = nodeCount / Math.max(1, fileCount);
    const nodeSensitivity = this.calculateSizeSensitivity(nodeCount);
    
    // Consider code structure complexity (relationships per node)
    const relationshipCount = understanding.relationships.length;
    const relationshipDensity = relationshipCount / Math.max(1, nodeCount);
    
    // Calculate combined sensitivity factor
    // Higher value = more sensitive thresholds for small codebases
    const combinedSensitivity = (fileSensitivity * 0.6) + (nodeSensitivity * 0.3) + (relationshipDensity * 0.1);
    
    // Define adaptive thresholds for different aspects
    return {
      // Concept extraction thresholds (lower = more concepts extracted)
      fileSignificanceMultiplier: Math.max(0.5, 1.0 - (combinedSensitivity * 0.5)),
      conceptCountThreshold: Math.max(1, 2 - Math.round(combinedSensitivity)),
      conceptSignificanceThreshold: Math.max(0.02, 0.05 - (combinedSensitivity * 0.03)),
      
      // Documentation extraction (lower = more concepts from docs)
      documentationCountThreshold: Math.max(1, 2 - Math.round(combinedSensitivity * 1.5)),
      
      // Semantic unit coherence (lower = more units created)
      semanticUnitCoherenceThreshold: Math.max(0.3, 0.5 - (combinedSensitivity * 0.2)),
      
      // Relationship thresholds (lower = more relationships detected)
      relationshipSimilarityThreshold: Math.max(0.1, 0.2 - (combinedSensitivity * 0.1)),
      semanticSimilarityThreshold: Math.max(0.3, 0.6 - (combinedSensitivity * 0.3)),
      
      // Overall sensitivity multiplier
      sensitivityMultiplier: combinedSensitivity
    };
  }
  
  private calculateSizeSensitivity(count: number): number {
    // Sigmoid-like function that maps count to sensitivity between 0 and 1
    // Small counts get high sensitivity, large counts get low sensitivity
    if (count <= 10) return 1.0;  // Very small codebases get maximum sensitivity
    if (count >= 1000) return 0.1; // Very large codebases get minimum sensitivity
    
    // Logarithmic scale for everything in between
    return 1.0 - (Math.log10(count) * 0.3);
  }
  */
